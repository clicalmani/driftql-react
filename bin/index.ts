...
export class QueryBuilder<T> {
  ...
  protected wheres: DriftQL.WhereClause[] = [];
  protected orders: DriftQL.OrderClause[] = [];
  protected joins: DriftQL.JoinClause[] = [];
  ...
  where(condition: DriftQL.WhereClause): this {
  ...
  join(clause: DriftQL.JoinClause): this {
...
export abstract class Elegant {
  ...
  static where<T extends Elegant>(condition: DriftQL.WhereClause): QueryBuilder<T> {
    const builder = new QueryBuilder<T>(this.resourceName);
